...
function renderElement(data) {
    ...
    data.forEach((elem) => {
        ...
        cloneTemplate.querySelector("#image").src = elem.imageURL;
        cloneTemplate.querySelector("#image").onerror = (evt) => {
            evt.target.src = "http://via.placeholder.com/100x150";
        };
        cloneTemplate.querySelector(".card-title").textContent = elem.name;
        cloneTemplate.querySelector(".card-text").textContent = elem.steps.toString().split(" ").slice(0, 15).join(" ")+"...";
        cloneTemplate.querySelector(".btn-primary").href = elem.originalURL;
